...
1. At the PSP main menu, press Select. In the menu that pops up (only on [custom firmware](/docs/reference/psp-custom-firmware)), set the USB Device to UMD Disc.
1. Still in the PSP menu, choose USB Connection in the Settings menu (to the left).
1. On your PC, a folder will open automatically, containing a virtual ISO file representing the disc contents.
   To copy the game to your PC, simply drag this file to somewhere on your hard drive and the copy will start.
1. When the copy is done, rename the file to the name of the game, and it's ready for use.

## Compressed ISO formats

PPSSPP can play files directly from these formats without decompressing them first, which makes them much more practical than keeping your ISO files inside ZIP or RAR archives.
...
CHD is supported from PPSSPP 1.17, and stands for Compressed Hard Drive.
It's MAME's format, originally made for archiving arcade game hard drives, but later repurposed for disc-based systems.

It has a slightly better compression ratio than CSO, but to perform optimally, PSP ISOs should be compressed CHD in the DVD mode (`createdvd`).
...
`createdvd` is what you want to use for best performance.
...
If you have digital downloads on your real PSP, they can be used directly on PPSSPP.
Just copy the folder with the `EBOOT.PBP` over.
Note that this has not been tested as much as ISO loading so there may still be issues with some games.
