...
Find out where `PPSSPPWindows64.exe` is located.
(That's the one you're running, right? Don't use the 32-bit one unless you have to!)
...
Then, create a file called `installed.txt` if it's not already there.
