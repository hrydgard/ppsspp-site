...
1. Start by creating a `textures.ini` file. This can be done automatically: load your game, press ESC, go to `Settings->Tools->Developer Tools->Texture Replacement`, and choose `Create/Open textures.ini file for your game`.
2. In the ini file, set `hash="xxh64"` and `ignoreAddress="true"` for a good tradeoff between robustness and speed.
3. Enable `[X] Save textures` and play the game to dump textures.
4. Dumped textures are written to the `new` subfolder in your texture pack.
5. Move them from the `new` folder into the main texture pack folder (or subfolders if you reference those in the ini), then edit them.
6. Restart the game with texture replacement enabled to verify replacements.
...
## Hash behavior, texture size quirks, and mipmaps
...
When a game wants to show a full screen image (480x272), it has to use a texture that's 512x512, because the PSP hardware just can't process a texture actually 480x272 in size. The extra space that isn't used is usually garbage.
...
Because of how GPUs work, they only pick a few pixels from the 1920x1080 image to draw on screen. You might get unlucky and mostly sample tail-lights, so the car becomes a red blur when far away (even if the actual car is black.)

Mipmapping fixes this by giving the GPU smaller images for farther away. Some, but not all, PSP games use these.

The "original size" image is "mip level 0", and every size after that must be exactly half the width and height. So if "mip level 0" is 1920x1080, "mip level 1" should be 960x540.
...
KTX2 and DDS files can include multiple mipmaps in one file (see the image format section below.) You don't need a full set of mip levels. Even a few often help, especially if the player has anisotropic texturing enabled.

### Filter mode override

As documented in the ini file example above, texture filtering can be overridden on a per-hash basis.

This works even if you don't actually replace the texture. This can be useful for enforcing nearest filtering on things that look good pixelated, for example.
...
If you want to stick with .png for compatibility with very old PPSSPP versions, or just the ease of use, using pngquant / pngyu to reduce the size of PNGs will also make PNG files load faster. See [this post](https://github.com/hrydgard/ppsspp/issues/12059#issuecomment-1436104606) for reference.
...
For textures that are not supposed to wrap around (like title screens, menu pieces and stuff), also pass the flag `-mip_clamp` for optimal results. basisu has a large number of parameters that can be experimented with.
