...
PPSSPP 1.20.3 is rolling out now, with some important bugfixes, including the iOS multitouch bug and background setting bugs. 1.20.2 also introduces an improved ad hoc server list.
...
For the detailed list of changes since the 1.19.x series, and the 1.20.x updates, see the [news item](/news/release-1.20).
...
Relay servers, or "aemu postoffice", are a new innovation in the PSP ad hoc multiplayer space - they make it far easier to configure, since you no longer have to mess with opening ports in your firewall etc. These are now automatically used for popular servers, and for other servers you can choose whether to try to use them or not.
