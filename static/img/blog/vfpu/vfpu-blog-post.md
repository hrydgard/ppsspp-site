# Reading the PSP's math unit off its own outputs

The PSP's vector unit, the VFPU, has instructions for reciprocals, square roots, exp2, log2, sine, cosine and arcsine. None of them return the correctly rounded IEEE result. Each returns its own approximation, truncated to 22 bits. Mostly nobody cares, but games that replay inputs, check sums or sync over the network can, so an emulator wants the exact bits.

PPSSPP has had them bit-exact for a while, thanks to fp64's work in [issue #16946](https://github.com/hrydgard/ppsspp/issues/16946). The method was careful measurement: an easy-to-compute base approximation, plus per-64-input correction deltas, plus lists of exceptions. It shipped as about 4.9 MB of `.dat` files. That's the right way to get correct answers when you don't know the algorithm. It also leaves an itch: the hardware certainly doesn't have 5 MB of ROM for this. What does it actually do?
...
![Sawtooth of per-interval errors for rcp, exp2, sqrt and rsqrt](fig1-sawtooth.svg)
...
![Squared-term coefficient per segment for rsqrt, rcp, exp2 and sqrt](fig2-coefficients.svg)
...
![The squarer output recovered from data, against ceil(t squared over 256) times 256](fig3-squarer.svg)
...
![log2 output step by input exponent](fig4-log2-step.svg)
...
Thanks to fp64, whose exact tables made the whole thing possible. A reverse-engineered model needs an exact target, and that work was the hard part.
